...
public sealed partial class MegafaunaSystem : EntitySystem
{
    ...
    private readonly List<(TimeSpan Time, MegafaunaSelector Selector)> _dueSelectors = new();
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var ai))
        {
            ...
            foreach (var (time, action) in ai.Schedule)
            {
                ...
                _dueSelectors.Add((time, action));
            }

            foreach (var (time, action) in _dueSelectors)
            {
                ...
                ai.Schedule[nextAction] = ai.Selector;
            }
        }
    }
