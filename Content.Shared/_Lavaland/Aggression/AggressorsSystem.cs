...
public sealed class AggressorsSystem : EntitySystem
{
    ...
    private readonly List<EntityUid> _aggressorsToRemove = new();
    private readonly List<EntityUid> _aggressivesToClean = new();
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var aggressive, out var xform))
        {
            if (aggressive.ForgiveRange == null
                || curTime < aggressive.NextUpdate)
                continue;

            aggressive.NextUpdate = curTime + aggressive.UpdateDelay;

            _aggressorsToRemove.Clear();
            ...
            foreach (var aggressor in aggressive.Aggressors)
            {
                ...
                if (distance > aggressive.ForgiveRange
                    ...
                    _aggressorsToRemove.Add(aggressor);
            }

            foreach (var remove in _aggressorsToRemove)
            {
    ...
    {
        var (uid, comp) = ent;
        if (!comp.Aggressors.Add(aggressor))
            return;
    ...
    public void CleanAggressions(Entity<AggressorComponent?> aggressor)
    {
        ...
        _aggressivesToClean.Clear();
        foreach (var aggressive in aggressor.Comp.Aggressives)
        {
            _aggressivesToClean.Add(aggressive);
        }

        foreach (var aggressive in _aggressivesToClean)
        {
