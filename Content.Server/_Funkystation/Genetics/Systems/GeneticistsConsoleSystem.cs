...
public sealed class GeneticistsConsoleSystem : EntitySystem
{
    ...
    {
        if (comp.InjectorPrintCooldownEnd is { } cooldownEnd && cooldownEnd > _timing.CurTime)
        {
            _popup.PopupEntity(Loc.GetString("dna-scanner-print-cooldown-popup",
                ("seconds", (int)Math.Ceiling((cooldownEnd - _timing.CurTime).TotalSeconds))), uid);
            _audio.PlayPvs(comp.SoundDeny, uid);
        ...
        comp.InjectorPrintCooldownEnd = _timing.CurTime + TimeSpan.FromSeconds(InjectorPrintCooldownSeconds);
        Dirty(uid, comp);
        SendUiUpdate(uid, comp, false);
    ...
    private GeneticistsConsoleBoundUserInterfaceState BuildUiState(EntityUid uid, GeneticistsConsoleComponent comp, bool fullUpdate)
    {
        ...
        return new GeneticistsConsoleBoundUserInterfaceState(
            ...
            injectorPrintCooldownEnd: comp.InjectorPrintCooldownEnd
        );
    }
