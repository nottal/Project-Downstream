...
public sealed partial class GeneticistsConsoleWindow : FancyWindow
{
    ...
    internal TimeSpan? InjectorPrintCooldownEnd { get; private set; }
    ...
    public void UpdateInjectorPrintCooldown(TimeSpan? cooldownEnd)
    {
        InjectorPrintCooldownEnd = cooldownEnd;
        _enzymesView.UpdateInjectorPrintCooldown();
        _storageView.UpdateInjectorPrintCooldown();
    }

    public bool IsInjectorPrintOnCooldown(out int secondsRemaining)
    {
        secondsRemaining = 0;

        if (InjectorPrintCooldownEnd is not { } cooldownEnd || cooldownEnd <= _timing.CurTime)
            return false;

        secondsRemaining = (int)Math.Ceiling((cooldownEnd - _timing.CurTime).TotalSeconds);
        return true;
    }
}
